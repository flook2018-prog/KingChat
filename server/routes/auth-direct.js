...
console.log('🔐 Auth routes loading with direct database connection');

const JWT_SECRET = process.env.JWT_SECRET || 'your-very-secure-secret-key-for-kingchat-2024';
const JWT_EXPIRES_IN = '24h';

// POST /api/auth/login - Login with database authentication
router.post('/login', async (req, res) => {
  try {
    ...
    console.log(`✅ Authentication successful from database: ${username} (ID: ${user.id})`);
...
router.get('/me', async (req, res) => {
  try {
    ...
    console.log(`🔍 Getting user info from database: ${decoded.username}`);
    ...
    const user = result.rows[0];
    console.log(`✅ Retrieved user info from database: ${user.username}`);
